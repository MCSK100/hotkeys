...
export function WeatherPicker({ value, onChange, small }: { value: WeatherId; onChange: (w: WeatherId) => void; small?: boolean }) {
  return (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      {!small && <span className="mr-1 text-[11px] font-medium text-white/50">Track</span>}
      {WEATHER_IDS.map((id) => (
        <button key={id} onClick={() => onChange(id)} title={WEATHERS[id].name}
          className={`rounded-full font-medium transition ${small ? 'px-3 py-1.5 text-[11px]' : 'px-4 py-2 text-[12px]'} ${value === id ? 'bg-white text-black' : 'border border-white/15 text-white/60 hover:border-white/40 hover:text-white'}`}>
          {WEATHERS[id].name}
        </button>
      ))}
    </span>
